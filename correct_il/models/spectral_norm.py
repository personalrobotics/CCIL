...
class SpectralNorm:
    ...
    eps: float
    lipschitz_constraint: float
    

    def __init__(self, name: str = 'weight', n_power_iterations: int = 1, dim: int = 0, eps: float = 1e-12, lipschitz_constraint: float = 1.0) -> None:
        self.name = name
        self.dim = dim
    ...
    def compute_weight(self, module: Module, do_power_iteration: bool) -> torch.Tensor:
        ...
        weight = weight / torch.max(sigma/self.lipschitz_constraint,torch.tensor(1.0).to(sigma.device))
        return weight
    ...
    @staticmethod
    def apply(module: Module, name: str, n_power_iterations: int, dim: int, eps: float, lipschitz_constraint: float) -> 'SpectralNorm':
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNorm) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                   "the same parameter {}".format(name))

        fn = SpectralNorm(name, n_power_iterations, dim, eps, lipschitz_constraint=lipschitz_constraint)
        weight = module._parameters[name]
        # TODO: not supported in Torch 1.7
...
def apply_spectral_norm(module: T_module,
                  name: str = 'weight',
                  n_power_iterations: int = 1,
                  eps: float = 1e-12,
                  lipschitz_constraint: float = 1.0,
    ...
    Args:
        ...
        eps (float, optional): epsilon for numerical stability in
            calculating norms
        lipschitz_constraint (float, optional): The lipschitz constraint for the spectral norm.
    ...
    if dim is None:
        if isinstance(module, (torch.nn.ConvTranspose1d,
            ...
            dim = 0
    SpectralNorm.apply(module, name, n_power_iterations, dim, eps, lipschitz_constraint)
    return module
